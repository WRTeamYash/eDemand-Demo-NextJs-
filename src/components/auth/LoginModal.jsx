...
const LoginModal = ({ open, close, setOpenProfileModal }) => {
  ...
  // Handle Backspace for smooth UX
  const handleOtpKeyDown = (e, index) => {
    if (e.key === "Backspace") {
      // Handle backspace (already implemented)
      ...
      // Check if the last OTP input field is filled
      if (index === otp.length - 1 && otp.every((digit) => digit !== "")) {
        // If all OTP fields are filled and we're at the last field
        verifyOtp(); // Trigger the verify OTP function
      }
    }
  };
